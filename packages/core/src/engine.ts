...
import type { AffinityStage, BattleState, CombatStats, DamageType, EnemyDefinition, EngineResult, FocusTimeInput, GameContent, GameRules, ItemDefinition, PlayMode, SPPetEvent, SPPetEventType, SPPetState, TaskCompletedInput } from './types.js';

export const DEFAULT_RULES: Required<GameRules> = { commissionTaskTarget: 3, commissionTaskXp: 30, commissionFocusTarget: 60, commissionFocusXp: 30, commissionPriorityXp: 20, commissionReviewXp: 20, dailyXpCap: 100, disconnectDecayMinutes: 60, disconnectDecayAmount: 2 };
...
export const createInitialState = (now?: Date | string | number): SPPetState => {
  ...
  return { version: 4, mode: 'adventure', level: 1, xp: 0, coins: 0, streak: 0, lastActiveDate: null, lastLoginDate: day, totalTasksCompleted: 0, totalFocusMinutes: 0, totalBattlesWon: 0, today: { date: day, tasksCompleted: 0, xpEarned: 0, focusRewardSteps: 0 }, processedTaskIds: [], observedFocusMinutesByTask: {}, commissions: commissionsFor(day, rules), checkIn: { lastDate: null, streak: 0 }, pet: { name: 'PRTS-β', condition: 100, hp: 100, baseStats: { maxHp: 100, attack: 12, defense: 5, resistances: { ...ZERO_RESISTANCE } }, learnedSkills: [...STARTER_SKILLS], equippedSkills: [...STARTER_SKILLS], affinity: { points: 0, stage: 'stranger', touchDate: null, touchesToday: 0 }, inventory: {}, equipped: { weapon: null, armor: null, accessory: null }, skinPart: null, buff: { xpBonus: 0, coinBonus: 0, attackBonus: 0, battlesRemaining: 0 }, lastConnectedAt: date.toISOString(), lastConditionDecayAt: date.toISOString() }, adventure: { chapterIndex: 0, encounterIndex: 0, claimedMapRewards: [], activeBattle: null }, updatedAt: date.toISOString() };
};
...
export const onDayChecked = (current: SPPetState, occurredAt?: Date | string | number, rulesInput?: GameRules): EngineResult => {
  ...
  if (state.today.date !== day) { state.today = { date: day, tasksCompleted: 0, xpEarned: 0, focusRewardSteps: 0 }; state.commissions = commissionsFor(day, rules); changed = true; }
...
const settleVictory = (state: SPPetState, battle: BattleState, enemy: EnemyDefinition, now: Date, events: SPPetEvent[]): void => {
  if (battle.rewardsClaimed) return; battle.rewardsClaimed = true; battle.phase = 'story_after'; battle.storyIndex = 0; const requestedXp = Math.round(enemy.xp * (1 + state.pet.buff.xpBonus / 100)); const xp = awardXp(state, requestedXp, now); const coins = Math.round(enemy.coins * (1 + state.pet.buff.coinBonus / 100)); state.coins += coins; state.totalBattlesWon += 1; consumeBattleBuff(state); events.push(event('BATTLE_WON', { enemy: enemy.name, xp: xp.awarded, coins, boss: Boolean(enemy.isBoss) }, now), ...xp.events);
};
