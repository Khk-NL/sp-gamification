...
const settleVictory = (state: SPPetState, battle: BattleState, enemy: EnemyDefinition, now: Date, events: SPPetEvent[]): void => {
  if (battle.rewardsClaimed) return; battle.rewardsClaimed = true; battle.phase = 'story_after'; battle.storyIndex = 0; const statusMultiplier = conditionRewardMultiplier(state.pet.condition); const requestedXp = Math.round(enemy.xp * (1 + state.pet.buff.xpBonus / 100) * statusMultiplier); const xp = awardXp(state, requestedXp, now); const coins = Math.round(enemy.coins * (1 + state.pet.buff.coinBonus / 100) * statusMultiplier); state.coins += coins; state.totalBattlesWon += 1; consumeBattleBuff(state); events.push(event('BATTLE_WON', { enemy: enemy.name, xp: xp.awarded, coins, boss: Boolean(enemy.isBoss), statusMultiplier }, now), ...xp.events);
};
