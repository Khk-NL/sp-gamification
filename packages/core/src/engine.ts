...
export const hydrateState = (input: unknown, now?: Date | string | number, rulesInput?: GameRules): SPPetState => {
  ...
  return {
    version: 3, level: Math.max(1, nonNegativeInt(value.level, 1)), xp: nonNegativeInt(value.xp), coins: nonNegativeInt(value.coins), streak: nonNegativeInt(value.streak), lastActiveDate: typeof value.lastActiveDate === 'string' ? value.lastActiveDate : null,
...
export const onDayChecked = (current: SPPetState, occurredAt?: Date | string | number, rulesInput?: GameRules): EngineResult => {
  const now = asDate(occurredAt); const state = hydrateState(current, now, rulesInput); const day = localDateKey(now); const rules = rulesOf(rulesInput); let changed = false;
...
export const onTaskCompleted = (current: SPPetState, input: TaskCompletedInput): EngineResult => {
  const now = asDate(input.occurredAt); const rules = rulesOf(input.rules); const state = onDayChecked(current, now, rules).state;
  ...
  const events = [event('TASK_COMPLETED', { taskId: input.taskId, progress: state.commissions.tasks.progress, target: state.commissions.tasks.target }, now)]; if (first) events.push(event('STREAK_UPDATED', { streak: state.streak }, now)); events.push(...claimCommissions(state, rules, now)); touch(state, now); return { state, events };
};
...
export const onFocusTimeAdded = (current: SPPetState, input: FocusTimeInput): EngineResult => {
  const now = asDate(input.occurredAt); const rules = rulesOf(input.rules); const state = onDayChecked(current, now, rules).state; let added = nonNegativeInt(input.minutes);
  ...
  if (!added) return { state, events: [] }; state.totalFocusMinutes += added; state.commissions.focus.progress += added; const events = claimCommissions(state, rules, now); touch(state, now); return { state, events };
};

export const checkIn = (current: SPPetState, occurredAt?: Date | string | number): EngineResult => {
  const now = asDate(occurredAt); const state = onDayChecked(current, now).state; const day = localDateKey(now); if (state.checkIn.lastDate === day) return { state, events: [] };
  const distance = state.checkIn.lastDate ? dateDistance(state.checkIn.lastDate, day) : null; state.checkIn.streak = distance === 1 ? state.checkIn.streak + 1 : 1; state.checkIn.lastDate = day; const coins = 5 + Math.min(7, state.checkIn.streak) * 2; state.coins += coins; touch(state, now); return { state, events: [event('CHECK_IN', { streak: state.checkIn.streak, coins }, now)] };
};
...
export const getComputedStats = (stateInput: SPPetState, contentInput?: GameContent): CombatStats => {
  ...
  if ((setCounts.get('frostline') ?? 0) >= 2) stats.damageBonus.ice = (stats.damageBonus.ice ?? 0) + 15;
...
export const endTurn = (current: SPPetState, contentInput?: GameContent, occurredAt?: Date | string | number): EngineResult => {
  ...
  if (intent.kind === 'guard') { const type = intent.type ?? enemy.element ?? 'physical'; if (battle.enemyBlock > 0 && battle.enemyBlockType !== type) battle.enemyBlock = 0; battle.enemyBlock += intent.value; battle.enemyBlockType = type; battle.log.unshift(`${enemy.name} 获得 ${intent.value} 点 ${type} 护盾；同属性攻击伤害 ×0.50。`); events.push(event('ENEMY_ACTION', { enemy: enemy.name, action: 'guard', value: intent.value, type, sameTypeReduction: 50 }, now)); }
