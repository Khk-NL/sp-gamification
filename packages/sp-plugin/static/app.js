...
function renderBattle() {
  ...
  $('enemy').classList.toggle('visible', Boolean(battle)); document.querySelector('.enemy-data').classList.toggle('visible', Boolean(battle)); $('deploy').style.display = battle ? 'none' : 'block'; $('player-hp').textContent = `${state.pet.hp} HP`;
  if (!battle) { $('skills').replaceChildren(); $('battle-log').textContent = language() === 'en' ? 'Select Adventure to deploy.' : '进入冒险后开始部署。'; $('turn').textContent = '--'; $('resource').textContent = '--'; $('story').classList.remove('open'); return; }
  const enemy = currentEnemyDefinition(); $('enemy-name').textContent = enemy ? enemyName(enemy) : battle.enemyName; $('boss-tag').textContent = enemy?.isBoss ? 'MINI BOSS' : 'TARGET'; $('enemy-hp').textContent = `${battle.enemyHp} / ${battle.enemyMaxHp} HP`; $('enemy-hp-bar').style.width = percent(battle.enemyHp, battle.enemyMaxHp); $('turn').textContent = battle.turn; $('resource').textContent = `${battle.resource} / ${battle.maxResource}`;
  ...
  const skills = model.content.skills.filter((skill) => state.pet.learnedSkills.includes(skill.id)); $('skills').replaceChildren(...skills.map((skill) => { const button = document.createElement('button'); button.className = 'skill'; const title = document.createElement('b'); title.textContent = `${itemName(skill)} [${skill.cost}]`; const description = document.createElement('small'); description.textContent = skill.description; button.append(title, description); button.disabled = battle.phase !== 'combat' || skill.cost > battle.resource; button.addEventListener('click', () => act('useSkill', { skillId: skill.id })); return button; }));
...
$('deploy').addEventListener('click', () => act('startBattle'));
