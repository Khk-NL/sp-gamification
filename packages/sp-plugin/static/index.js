...
function itemAction(item) {
  ...
  if (['food','medicine','consumable'].includes(item.kind)) return { label: language() === 'en' ? `USE ×${count}` : `使用 ×${count}`, action: 'useItem' };
...
function renderShop() {
  ...
  const rotation = model.state.shop?.rotation || []; model.content.items.filter((item) => (!rotation.length || rotation.includes(item.id)) && (activeFilter === 'all' || item.kind === activeFilter)).forEach((item) => {
...
function renderInventory() {
  const root = $('inventory'), owned = model.content.items.filter((item) => (model.state.pet.inventory[item.id] || 0) > 0); root.replaceChildren(...(owned.length ? owned : [{ id:'', name:'背包为空', nameEn:'Inventory empty', description:'', icon:'--' }]).map((item) => { const count = model.state.pet.inventory[item.id] || 0, level = model.state.pet.equipmentLevels?.[item.id] || 0; const card = document.createElement('article'); card.className = 'item'; card.innerHTML = `<div class="item-icon">${item.icon}</div><b>${itemName(item)}${level ? ` · +${level}` : ''}</b><small>${item.description || ''}</small>`; if (item.id) { const action = document.createElement('button'); if (item.slot && model.state.pet.equipped[item.slot] === item.id) { action.textContent = level >= 3 ? 'MAX' : `${language() === 'en' ? 'UPGRADE' : '强化'} ◈ ${(level + 1) * 25}`; action.disabled = level >= 3 || model.state.coins < (level + 1) * 25; action.addEventListener('click', () => act('upgradeEquipment', { itemId: item.id })); } else if (['food','medicine','consumable'].includes(item.kind)) { action.textContent = `${language() === 'en' ? 'USE' : '使用'} ×${count}`; action.addEventListener('click', () => act('useItem', { itemId: item.id })); } else { action.textContent = language() === 'en' ? 'EQUIP' : '装备'; action.addEventListener('click', () => act('equipItem', { itemId: item.id })); } card.append(action); } return card; }));
}
...
function displayedDamage(skill, battle = null) {
  ...
  const level = model.state.pet.skillLevels?.[skill.id] || 1, power = Math.min(1.5, skill.power + (level - 1) * .5);
  let multiplier = 1;
  ...
  return Math.max(1, Math.round(model.computedStats.attack * power * (1 + bonus / 100) * multiplier) - defense);
}
...
function setShieldEffect(element, type) { element.classList.remove(...elementClasses.map((entry) => `shield-${entry}`)); if (type) element.classList.add(`shield-${visualEffect(type, 'shieldEffect')}`); }
...
function renderMap() {
  ...
  chapter.enemies.forEach((enemy, index) => {
    const floor = document.createElement('section'); floor.className = `map-floor floor-${index}`; const label = document.createElement('span'); label.className = 'floor-label'; label.textContent = enemy.isBoss ? 'BOSS' : `FLOOR ${index + 1}`; floor.append(label); const rewardId = `${chapter.id}:${index}`, choseReward = state.adventure.claimedMapRewards.includes(rewardId), passed = index < state.adventure.encounterIndex, available = index === state.adventure.encounterIndex;
    const battle = document.createElement('button'); battle.className = `map-node battle ${enemy.isBoss ? 'boss' : ''} ${passed && !choseReward ? 'cleared' : passed ? 'bypassed' : available ? 'available' : 'locked'}`; battle.dataset.node = 'battle'; battle.dataset.index = String(index); battle.disabled = !available; battle.innerHTML = `<i>${enemy.isBoss ? '♛' : '⚔'}</i><b>${enemyName(enemy)}</b><small>${String(enemy.element || 'physical').toUpperCase()} · HP ${enemy.maxHp} · XP ${enemy.xp} · ◈ ${enemy.coins}</small>`; floor.append(battle);
...
function renderBattleEffects(battle) {
  const effects = []; const buff = model.state.pet.buff; if (buff.attackBonus) effects.push(`玩家增益：攻击 +${buff.attackBonus}，剩余 ${buff.battlesRemaining} 场`); if (buff.xpBonus) effects.push(`玩家增益：战斗 XP +${buff.xpBonus}%`); if (buff.coinBonus) effects.push(`玩家增益：战斗金币 +${buff.coinBonus}%`); if (battle.playerBlock) effects.push(`玩家护盾：${battle.playerBlock} 点 ${battle.playerBlockType || 'physical'}`); if (battle.enemyBurn) effects.push(`敌方减益：回合结束受到 ${battle.enemyBurn} 点火伤`); if (battle.enemyWeaken) effects.push(`敌方减益：本回合攻击 -${battle.enemyWeaken}%`); if (battle.enemyAttackBuff) effects.push(`敌方增益：攻击 +${battle.enemyAttackBuff}`); if (battle.enemyBlock) effects.push(`敌方护盾：${battle.enemyBlock} 点 ${battle.enemyBlockType || 'physical'}`); $('battle-effects').replaceChildren(...(effects.length ? effects : ['当前无 Buff / Debuff']).map((text) => { const span = document.createElement('span'); span.textContent = text; return span; }));
}
...
document.querySelector('#skill-loadout').addEventListener('click', async (event) => { const upgrade = event.target.closest('[data-upgrade-skill]'); if (upgrade) { await act('upgradeSkill', { skillId: upgrade.dataset.upgradeSkill }); return; } const button = event.target.closest('[data-skill-id]'); if (!button) return; const current = [...(model.state.pet.equippedSkills || [])], skillId = button.dataset.skillId, next = current.includes(skillId) ? current.filter((id) => id !== skillId) : [...current, skillId].slice(0, 4); await act('setEquippedSkills', { skillIds: next }); });
...
$('end-turn').addEventListener('click', async () => { const beforeHp = model.state.pet.hp, beforeBattle = model.state.adventure.activeBattle, enemy = currentEnemyDefinition(), intent = beforeBattle && enemy?.intents[beforeBattle.intentIndex % enemy.intents.length]; await act('endTurn'); const afterBattle = model.state.adventure.activeBattle; if (model.state.pet.hp < beforeHp && intent?.kind === 'attack') playCombatEffect(document.querySelector('.pet-fighter'), intent.type || 'physical'); if (beforeBattle && afterBattle?.phase === 'story_after') playBattleCue('win', language() === 'en' ? 'VICTORY // Rewards secured' : '战斗胜利 // 奖励已结算'); else if (beforeBattle && !afterBattle) playBattleCue('lose', language() === 'en' ? 'DEFEAT // Pet recovered to 30% HP' : '战斗失败 // 宠物恢复至 30% HP'); });
