...
assert.match(uiHtml, /--journal-paper/);
assert.match(uiHtml, /Segoe Print/);
assert.match(uiHtml, /#7f9984/i);
...
const battle = await messageHandler({ type: 'startBattleAt', encounterIndex: 1 }); assert.equal(battle.state.adventure.activeBattle.phase, 'story_before'); assert.equal(battle.state.adventure.activeBattle.resource, 5); assert.equal(battle.state.pet.condition, 90);
const resolved = await messageHandler({ type: 'advanceStory', skip: true }); assert.equal(resolved.state.adventure.activeBattle.phase, 'story_after'); assert.equal(resolved.state.adventure.activeBattle.lastSkillId, 'strike');
const battleFrame = JSON.stringify(resolved.state.adventure.activeBattle); await hooks.get('taskComplete')(completed('task-4')); const afterLearning = await messageHandler({ type: 'getState' }); assert.equal(JSON.stringify(afterLearning.state.adventure.activeBattle), battleFrame);
await messageHandler({ type: 'resetState' }); await messageHandler({ type: 'debugAddCoins' }); await messageHandler({ type: 'debugAddCoins' });
const mapShop = await messageHandler({ type: 'openMapShop', shopIndex: 0 }); assert.equal(mapShop.state.adventure.activeMapShopIndex, 0);
const bought = await messageHandler({ type: 'purchaseMapItem', itemId: 'ember-chip' }); assert.equal(bought.state.pet.learnedSkills.includes('ember-shot'), true);
const leftShop = await messageHandler({ type: 'leaveMapShop' }); assert.equal(leftShop.state.adventure.encounterIndex, 1); assert.equal(leftShop.state.adventure.activeMapShopIndex, null);
assert.equal(JSON.parse(persisted.get('gamification-state-v1')).version, 9);
