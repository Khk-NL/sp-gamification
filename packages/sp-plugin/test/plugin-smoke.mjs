...
const uiTheme = await readFile(new URL('../dist/archive-theme.css', import.meta.url), 'utf8');
assert.match(uiHtml, /href="\.\/index\.css"/);
assert.match(uiHtml, /src="\.\/index\.js"/);
assert.doesNotMatch(uiScript, /PluginAPI\./);
assert.match(uiTheme, /#77e4ff/i);
const hooks = new Map();
