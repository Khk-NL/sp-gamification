...
assert.match(uiHtml, /Silkscreen/);
assert.match(uiHtml, /#63b84c/i);
assert.match(uiHtml, /data:image\/png;base64/);
assert.match(uiHtml, /data:font\/woff2;base64/);
...
const battle = await messageHandler({ type: 'startBattleAt', encounterIndex: 0 }); assert.equal(battle.state.adventure.activeBattle.phase, 'story_before');
