...
test('local registration, login session and admin content publishing work', async (context) => {
  ...
  const release = await fetch(`${base}/api/v1/releases/manifest`); const manifest = await release.json(); assert.equal(manifest.version, '0.12.0'); assert.match(manifest.sha256, /^[a-f0-9]{64}$/);
