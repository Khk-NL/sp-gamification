...
const statsHtml = () => { const state = snapshot?.state; if (!state) return '<span>等待 Super Productivity…</span>'; const profile = snapshot.characterProfile || state.pet, affinity = profile.affinity || state.pet.affinity, next = 100 + state.level * 25, stages = { stranger:'陌生', familiar:'熟悉', close:'亲近', trusted:'信赖', best_friend:'挚友' }; return `<strong>Lv.${state.level}</strong><span>XP ${state.xp} / ${next}</span><br><em>◆ ${profile.condition}</em><span> ♥ ${stages[affinity?.stage] || '陌生'} ${affinity?.points || 0}</span><br><span>${profile.mood || 'Normal'}</span><span> 🔥 ${state.streak} 天</span><span> ◈ ${state.coins}</span>`; };
...
function renderCharacterLayers(character, settings) { const groups = ['outfits', 'accessories', 'effects', 'weather'], settingKeys = { outfits:'outfitId', accessories:'accessoryId', effects:'effectId', weather:'weatherId' }; for (const group of groups) { let layer = petZone.querySelector(`[data-character-layer="${group}"]`); if (!layer) { layer = document.createElement('div'); layer.className = `character-layer layer-${group}`; layer.dataset.characterLayer = group; petZone.append(layer); } const selected = character?.layers?.[group]?.find((entry) => entry.id === settings?.[settingKeys[group]]); layer.textContent = selected?.resource?.kind === 'emoji' ? selected.resource.value : ''; layer.style.backgroundImage = selected?.resource?.kind === 'image' ? `url("${selected.resource.value}")` : ''; }
  let controls = document.getElementById('character-layer-controls'); if (!controls) { controls = document.createElement('div'); controls.id = 'character-layer-controls'; document.getElementById('character-list').insertAdjacentElement('afterend', controls); controls.addEventListener('change', () => { const layers = Object.fromEntries(groups.map((group) => [settingKeys[group], controls.querySelector(`[data-layer-select="${group}"]`)?.value || ''])); void window.petApi.setLayers(layers); }); } controls.replaceChildren(...groups.map((group) => { const label = document.createElement('label'); label.textContent = ({ outfits:'服装层', accessories:'挂件层', effects:'特效层', weather:'天气层' })[group]; const select = document.createElement('select'); select.dataset.layerSelect = group; const none = document.createElement('option'); none.value = ''; none.textContent = '无'; select.append(none, ...(character?.layers?.[group] || []).map((entry) => { const option = document.createElement('option'); option.value = entry.id; option.textContent = entry.name; return option; })); select.value = settings?.[settingKeys[group]] || ''; label.append(select); return label; })); }
function renderAi(ai) {
  ...
  document.getElementById('memory-enabled').checked = ai.settings?.longTermMemoryEnabled === true;
  const history = ai.history || [], log = document.getElementById('chat-log'), historyList = document.getElementById('history-list'), memoryList = document.getElementById('memory-list');
  ...
  memoryList.replaceChildren(...(ai.memories || []).map((entry) => { const item = document.createElement('article'); item.className = 'history-entry'; const time = document.createElement('time'); time.textContent = new Date(entry.createdAt).toLocaleString(); const body = document.createElement('div'); body.textContent = entry.summary; const button = document.createElement('button'); button.textContent = '删除'; button.dataset.memoryId = entry.id; item.append(time, body, button); return item; }));
}
function ensurePrivacyControls() { const save = document.getElementById('save-awareness'); for (const [id, text] of [['clipboard-enabled','允许未来的剪贴板互动（当前版本不读取）'], ['file-management-enabled','允许未来的文件整理（当前版本不操作）'], ['telemetry-enabled','允许匿名遥测（当前版本不采集）']]) { if (document.getElementById(id)) continue; const label = document.createElement('label'); label.className = 'check'; const input = document.createElement('input'); input.id = id; input.type = 'checkbox'; label.append(input, ` ${text}`); save.before(label); } if (!document.getElementById('privacy-summary')) { const summary = document.createElement('pre'); summary.id = 'privacy-summary'; summary.className = 'prompt-preview'; save.after(summary); } }
function renderAwareness(awareness) { if (!awareness) return; ensurePrivacyControls(); for (const [id, key] of [['current-window-enabled','currentWindowEnabled'], ['vision-enabled','visionEnabled'], ['clipboard-enabled','clipboardEnabled'], ['file-management-enabled','fileManagementEnabled'], ['telemetry-enabled','telemetryEnabled']]) document.getElementById(id).checked = awareness.settings?.[key] === true; const current = awareness.currentWindow; document.getElementById('current-window-result').textContent = current ? `${current.process || '未知进程'}\n${current.title || '无标题'}\n${new Date(current.capturedAt).toLocaleString()}` : '尚未读取'; }
function renderPrivacySummary(value) { const journal = value.productivity?.journal, ai = value.ai?.settings; document.getElementById('privacy-summary').textContent = [`本地处理：窗口标题 ${value.awareness?.settings?.currentWindowEnabled ? '已授权' : '关闭'}；日记 ${journal?.enabled ? '启用' : '关闭'}（AI读取 ${journal?.aiReadEnabled ? '已授权' : '关闭'}）`, `上传外部 API：AI文本 ${ai?.enabled ? '已启用' : '关闭'}；主动截图 ${value.awareness?.settings?.visionEnabled ? '已授权' : '关闭'}`, '当前不采集：剪贴板、文件操作、匿名遥测；排行榜上传由 SP 插件单独控制。'].join('\n'); }
...
function renderObservation(value) { if (!value) return; const labels = { focus:'专注', battle:'战斗', feeding:'喂食', mood:'心情', task:'任务', interaction:'互动' }; document.getElementById('observation-summary').textContent = `${value.day}\n${Object.entries(value.counts || {}).map(([key, count]) => `${labels[key] || key}：${count}`).join('\n')}\n\n共 ${value.events?.length || 0} 条本地事件`; }
function renderTts(value) { if (!value) return; const settings = value.settings || {}; document.getElementById('tts-provider').value = settings.provider || 'disabled'; document.getElementById('tts-endpoint').value = settings.endpoint || ''; document.getElementById('tts-model').value = settings.model || ''; document.getElementById('tts-voice').value = settings.voice || 'default'; }
function renderBehavior(value, profile) { if (!value) return; const settings = value.settings || {}; document.getElementById('day-night-enabled').checked = settings.dayNightEnabled !== false; document.getElementById('sleep-start').value = settings.sleepStartHour ?? 23; document.getElementById('wake-hour').value = settings.wakeHour ?? 7; document.getElementById('proactive-enabled').checked = settings.proactiveEnabled === true; document.getElementById('proactive-interval').value = settings.proactiveIntervalMinutes || 60; document.getElementById('hourly-enabled').checked = settings.hourlyChimeEnabled === true; document.getElementById('system-load-enabled').checked = settings.systemLoadAwareness === true; document.getElementById('random-events-enabled').checked = settings.randomEventsEnabled === true; document.getElementById('character-state-summary').textContent = `当前角色：${snapshot?.character?.manifest?.name || 'SPPet'}｜情绪 ${profile?.mood || 'Normal'}｜状态 ${profile?.condition ?? 100}｜RAM ${value.systemLoad || 0}%｜${value.asleep ? '睡眠中' : '清醒'}`; if (value.asleep && !anyPanelOpen()) setCharacterState('sleep'); else if (['idle','sleep','happy','sad','tired'].includes(characterState)) { const moodState = { Happy:'happy', Sad:'sad', Tired:'tired', Excited:'happy', Normal:'idle' }; setCharacterState(moodState[profile?.mood] || 'idle'); } }
function render(next) {
  snapshot = next; idleLines = next.petProfile?.idleLines?.filter(Boolean) || idleLines; clickLines = next.petProfile?.clickLines?.filter(Boolean) || clickLines; renderSkin(next.skin); renderCharacter(next.character, next.skin); renderCharacterLayers(next.character, next.petSettings);
  ...
  renderPrivacySummary(next);
  renderFocus(next.focus);
  renderProductivity(next.productivity);
  renderObservation(next.observations);
  renderTts(next.tts);
  renderBehavior(next.behavior, next.characterProfile);
  for (const entry of next.events || []) { const feedback = entry.type === 'RECRUITMENT_RESULT' ? { message: `招募获得 ${entry.payload.name}（${entry.payload.rarity === 'rare' ? '稀有' : '普通'}）。`, state: entry.payload.rarity === 'rare' ? 'victory' : 'happy' } : entry.type === 'RECRUIT_TICKET_EARNED' ? { message: '获得 1 张招募券。', state: 'happy' } : eventFeedback(entry); if (feedback) messageQueue.push(feedback); } playNext();
}
...
document.getElementById('import-character').textContent = '导入角色 / .sppetpack';
...
document.getElementById('memory-enabled').addEventListener('change', async (event) => { await window.petApi.updateAiSettings({ longTermMemoryEnabled: event.target.checked }); document.getElementById('history-message').textContent = event.target.checked ? '长期记忆已启用，只检索本地记忆。' : '长期记忆已关闭，已有记忆仍保留。'; });
document.getElementById('memory-list').addEventListener('click', async (event) => { const button = event.target.closest('[data-memory-id]'); if (!button) return; await window.petApi.deleteMemory(button.dataset.memoryId); document.getElementById('history-message').textContent = '该条长期记忆已从 SQLite 删除。'; });
document.getElementById('clear-memories').addEventListener('click', async () => { if (!confirm('清空全部长期记忆？对话历史会保留。')) return; await window.petApi.clearMemories(); document.getElementById('history-message').textContent = '长期记忆已清空。'; });
document.getElementById('save-awareness').addEventListener('click', async () => { try { await window.petApi.updateAwarenessSettings({ currentWindowEnabled: document.getElementById('current-window-enabled').checked, visionEnabled: document.getElementById('vision-enabled').checked, clipboardEnabled: document.getElementById('clipboard-enabled').checked, fileManagementEnabled: document.getElementById('file-management-enabled').checked, telemetryEnabled: document.getElementById('telemetry-enabled').checked }); document.getElementById('settings-message').textContent = '系统感知授权已保存。'; } catch (error) { document.getElementById('settings-message').textContent = error.message || String(error); } });
...
document.getElementById('generate-observation').addEventListener('click', async () => { const message = document.getElementById('observation-message'), button = document.getElementById('generate-observation'); message.textContent = '正在根据今天的结构化事件生成…'; button.disabled = true; try { await window.petApi.generateObservationJournal(); message.textContent = '观察日记已生成，可在 AI 聊天或历史记录查看。'; } catch (error) { message.textContent = error.message || String(error); } finally { button.disabled = false; } });
document.getElementById('clear-observations').addEventListener('click', async () => { if (!confirm('清空最近 30 天的本地观察事件？')) return; await window.petApi.clearObservations(); document.getElementById('observation-message').textContent = '观察事件已清空。'; });
document.getElementById('save-tts').addEventListener('click', async () => { const message = document.getElementById('observation-message'); try { const key = document.getElementById('tts-key').value; if (key) await window.petApi.setTtsKey(key); await window.petApi.updateTtsSettings({ provider: document.getElementById('tts-provider').value, endpoint: document.getElementById('tts-endpoint').value, model: document.getElementById('tts-model').value, voice: document.getElementById('tts-voice').value }); document.getElementById('tts-key').value = ''; message.textContent = 'TTS 设置已保存。'; } catch (error) { message.textContent = error.message || String(error); } });
const playSpeech = async (dataUrl) => { const audio = new Audio(dataUrl), context = new AudioContext(), source = context.createMediaElementSource(audio), analyser = context.createAnalyser(), samples = new Uint8Array(analyser.frequencyBinCount); source.connect(analyser); analyser.connect(context.destination); setCharacterState('talk'); const animate = () => { analyser.getByteTimeDomainData(samples); const amplitude = samples.reduce((sum, value) => sum + Math.abs(value - 128), 0) / samples.length; petZone.classList.toggle('tts-mouth-open', amplitude > 4); if (!audio.paused && !audio.ended) requestAnimationFrame(animate); }; audio.addEventListener('ended', () => { petZone.classList.remove('tts-mouth-open'); setCharacterState('idle'); context.close(); }); await audio.play(); animate(); };
document.getElementById('speak-latest').addEventListener('click', async () => { const message = document.getElementById('chat-message'), latest = snapshot?.ai?.history?.at(-1); if (!latest) { message.textContent = '还没有可朗读的回答。'; return; } message.textContent = '正在生成语音…'; try { await playSpeech(await window.petApi.speak(latest.assistant)); message.textContent = ''; } catch (error) { message.textContent = error.message || String(error); } });
document.getElementById('save-behavior').addEventListener('click', async () => { const message = document.getElementById('behavior-message'); try { await window.petApi.updateBehaviorSettings({ dayNightEnabled: document.getElementById('day-night-enabled').checked, sleepStartHour: Number(document.getElementById('sleep-start').value), wakeHour: Number(document.getElementById('wake-hour').value), proactiveEnabled: document.getElementById('proactive-enabled').checked, proactiveIntervalMinutes: Number(document.getElementById('proactive-interval').value), hourlyChimeEnabled: document.getElementById('hourly-enabled').checked, systemLoadAwareness: document.getElementById('system-load-enabled').checked, randomEventsEnabled: document.getElementById('random-events-enabled').checked }); message.textContent = '作息与主动行为设置已保存。'; } catch (error) { message.textContent = error.message || String(error); } });
document.getElementById('wake-pet').addEventListener('click', async () => { await window.petApi.wakePet(); setCharacterState('idle'); document.getElementById('behavior-message').textContent = '今天已暂时唤醒；不会锁定任何功能。'; });
window.petApi.onBehaviorEvent((entry) => { if (entry?.message && !anyPanelOpen()) { const message = entry.type === 'PROACTIVE_BUBBLE' ? randomLine(idleLines, entry.message) : entry.message; messageQueue.push({ message, state: entry.type === 'RANDOM_EVENT' ? 'happy' : 'talk' }); playNext(); } });
