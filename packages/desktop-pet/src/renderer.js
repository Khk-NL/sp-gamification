...
const eventFeedback = (entry) => { const p = entry.payload || {}; if (entry.type === 'COMMISSION_COMPLETED') return { message: `委托完成，获得 ${p.xp} XP。` }; if (entry.type === 'LEVEL_UP') return { message: `升级至 Lv.${p.level}，新的能力已解锁。`, effect: 'win' }; if (entry.type === 'CHECK_IN') return { message: `签到补给已送达：${p.coins} 金币。` }; if (entry.type === 'MAP_REWARD_CLAIMED') return { message: `发现补给：${p.coins} 金币，恢复 ${p.heal} HP。`, effect: 'reward' }; if (entry.type === 'BATTLE_STARTED') return { message: `战斗开始！目标：${p.enemy || '未知敌人'}。`, effect: 'start' }; if (entry.type === 'BATTLE_WON') return { message: `战斗胜利！获得 ${p.xp} XP 和 ${p.coins} 金币。`, effect: 'win' }; if (entry.type === 'BATTLE_LOST') return { message: `宠物倒下了。状态下降至 ${p.condition}，恢复至 ${p.hp} HP。`, effect: 'lose' }; if (entry.type === 'SKILL_UPGRADED') return { message: `技能强化至 Lv.${p.level}，消耗 ${p.cost} 金币。`, effect: 'reward' }; if (entry.type === 'EQUIPMENT_UPGRADED') return { message: `${p.name} 强化至 +${p.level}。`, effect: 'reward' }; if (entry.type === 'ITEM_USED') return { message: `${p.name} 已使用。` }; if (entry.type === 'PET_CONDITION_CHANGED' && p.reason === 'missed_login') return { message: `漏登 ${p.missedDays} 天：状态 -${p.lost}${p.hpLost ? `，HP -${p.hpLost}` : ''}。`, effect: p.hpLost ? 'lose' : undefined }; if (entry.type === 'PET_CONDITION_CHANGED') return { message: `连接中断太久，状态下降了 ${p.lost}。` }; return null; };
