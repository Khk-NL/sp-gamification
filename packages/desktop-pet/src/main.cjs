...
const sendBridgeEvent = (type) => { const frame = encodeFrame(JSON.stringify({ type, occurredAt: new Date().toISOString() })); for (const socket of bridgeSockets) if (!socket.destroyed) socket.write(frame); };
const attachSocket = (socket) => {
  bridgeSockets.add(socket); bridgeConnected = true; publishSnapshot(); let buffer = Buffer.alloc(0);
